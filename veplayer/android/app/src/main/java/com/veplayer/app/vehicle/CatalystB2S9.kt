...
/** Catalyst temp bank 2 sensor 9 (OBD PID 0180), °C. */
object CatalystB2S9 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB2S9 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador B2S9 crítico. $c."
            "warn" -> "Cuidado. Catalizador B2S9 caliente. $c."
            else -> "Catalizador B2S9 a $c."
        }
    }
