...
/**
 * Short-term fuel trim bands (OBD PID 0106), signed %.
 */
object FuelTrimStft {
    data class State(
    ...
    fun evaluate(
        ...
        if (speed < minSpd) {
            return State(
                ...
                speedKmh = speed,
                band = "ok",
                label = if (absTrim >= 8f) "STFT · ${formatTrimPct(trim)}" else "",
            )
        }
    ...
    fun voicePhrase(st: State): String {
        ...
        return when (st.band) {
            "alert" -> "Atención. Corrección de combustible STFT crítica. $p. Revisa motor."
            "warn" -> "Cuidado. Corrección STFT fuera de rango. $p."
            else -> "Corrección STFT a $p."
        }
    }
