...
/** Absolute throttle position B (OBD PID 0147), %. */
object ThrottleB {
    ...
    fun evaluate(throttlePct: Float?, speedKmh: Float = 0f, warnPct: Float = 75f, alertPct: Float = 90f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(throttlePct = thr, speedKmh = speed, band = "ok", label = if (thr >= 45f) "ThrB · ${thr.toInt()}%" else "")
        }
        val band = when { thr >= alert -> "alert"; thr >= warn -> "warn"; else -> "ok" }
        return State(throttlePct = thr, speedKmh = speed, band = band, showWarn = band != "ok", label = "ThrB · ${thr.toInt()}%")
    }

    fun voicePhrase(st: State): String {
        val p = st.throttlePct?.toInt()?.let { "$it por ciento" } ?: "alta"
        return when (st.band) {
            "alert" -> "Atención. Mariposa B crítica. $p."
            "warn" -> "Cuidado. Mariposa B alta. $p."
            else -> "Mariposa B a $p."
        }
    }
