...
/**
 * Long-term fuel trim bands (OBD PID 0109), signed %.
 */
object FuelTrimLtftB2 {
    data class State(
    ...
    fun evaluate(
        ...
        if (speed < minSpd) {
            return State(
                ...
                speedKmh = speed,
                band = "ok",
                label = if (absTrim >= 8f) "LTB2 · ${formatTrimPct(trim)}" else "",
            )
        }
    ...
    fun voicePhrase(st: State): String {
        ...
        return when (st.band) {
            "alert" -> "Atención. Corrección de combustible LTFT B2 crítica. $p. Revisa motor."
            "warn" -> "Cuidado. Corrección LTFT B2 B2 fuera de rango. $p."
            else -> "Corrección LTFT B2 a $p."
        }
    }
