...
/** Fuel system 2 use % (OBD PID 019F byte C). */
object FuelSysUsePct2 {
    ...
    fun evaluate(usePct: Float?, speedKmh: Float = 0f, warnPct: Float = 70f, alertPct: Float = 85f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(usePct = p, speedKmh = speed, band = "ok", label = if (p >= warn - 5f) "FSu2 · ${p.toInt()}%" else "")
        }
        val band = when { p >= alert -> "alert"; p >= warn -> "warn"; else -> "ok" }
        return State(usePct = p, speedKmh = speed, band = band, showWarn = band != "ok", label = "FSu2 · ${p.toInt()}%")
    }

    fun voicePhrase(st: State): String {
        val p = st.usePct?.toInt()?.toString() ?: "alto"
        return when (st.band) {
            "alert" -> "Atención. Uso sistema combustible dos crítico. $p por ciento."
            "warn" -> "Cuidado. Uso sistema combustible dos alto. $p por ciento."
            else -> "Uso sistema combustible dos a $p por ciento."
        }
    }
