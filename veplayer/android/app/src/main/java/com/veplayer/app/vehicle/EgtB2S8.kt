...
/** EGT bank 2 sensor 8 (OBD PID 0199 bytes H/I), °C. */
object EgtB2S8 {
    ...
    fun evaluate(egtTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(egtTempC = c, band = band, showWarn = band != "ok", label = "EGTB2S8 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.egtTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. EGT B2S8 crítico. $c."
            "warn" -> "Cuidado. EGT B2S8 caliente. $c."
            else -> "EGT B2S8 a $c."
        }
    }
