...
/** Fuel pressure sensor A kPa (OBD PID 01C5 bytes A/B). */
object FuelPressA {
    ...
    fun evaluate(pressureKpa: Float?, speedKmh: Float = 0f, warnKpa: Float = 4000f, alertKpa: Float = 4800f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(pressureKpa = kpa, speedKmh = speed, band = "ok", label = if (kpa >= warn - 300f) "FPa · ${kpa.toInt()}" else "")
        }
        val band = when { kpa >= alert -> "alert"; kpa >= warn -> "warn"; else -> "ok" }
        return State(pressureKpa = kpa, speedKmh = speed, band = band, showWarn = band != "ok", label = "FPa · ${kpa.toInt()}")
    }

    fun voicePhrase(st: State): String {
        val k = st.pressureKpa?.toInt()?.let { "$it kPa" } ?: "alta"
        return when (st.band) {
            "alert" -> "Atención. Presión combustible A crítica. $k."
            "warn" -> "Cuidado. Presión combustible A alta. $k."
            else -> "Presión combustible A a $k."
        }
    }
