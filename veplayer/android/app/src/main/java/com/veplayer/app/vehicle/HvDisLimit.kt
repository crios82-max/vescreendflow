...
/** HEV discharge current limit A (OBD PID 01BA bytes D/E signed). */
object HvDisLimit {
    ...
    fun evaluate(currentA: Float?, speedKmh: Float = 0f, warnA: Float = 50f, alertA: Float = 25f, speedMinKmh: Float = 15f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(currentA = a, speedKmh = speed, band = "ok", label = if (a <= warn) "HvDis · ${a.toInt()}A" else "")
        }
        val band = when { a <= alert -> "alert"; a <= warn -> "warn"; else -> "ok" }
        return State(currentA = a, speedKmh = speed, band = band, showWarn = band != "ok", label = "HvDis · ${a.toInt()}A")
    }

    fun voicePhrase(st: State): String {
        val a = st.currentA?.toInt()?.let { "$it amperios" } ?: "bajo"
        return when (st.band) {
            "alert" -> "Atención. Límite descarga HV crítico. $a."
            "warn" -> "Cuidado. Límite descarga HV bajo. $a."
            else -> "Límite descarga HV a $a."
        }
    }
