...
object WwhObdContinuousMi {
    ...
    fun evaluate(miHours: Float?, warnH: Float = 24f, alertH: Float = 48f): State {
        ...
        return State(miHours = h, band = band, showWarn = band != "ok", label = "WwhCMI · ${h.toInt()}h")
    }

    fun voicePhrase(st: State): String {
        val h = st.miHours?.toInt()?.let { "$it horas" } ?: "elevadas"
        return when (st.band) {
            "alert" -> "Atención. Contador MI continuo WWH crítico. $h."
            "warn" -> "Cuidado. Contador MI continuo WWH alto. $h."
            else -> "MI continuo WWH a $h."
        }
    }
