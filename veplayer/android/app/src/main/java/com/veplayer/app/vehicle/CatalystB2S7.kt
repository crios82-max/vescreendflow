...
/** Catalyst temp bank 2 sensor 7 (OBD PID 017C), °C. */
object CatalystB2S7 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB2S7 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador B2S7 crítico. $c."
            "warn" -> "Cuidado. Catalizador B2S7 caliente. $c."
            else -> "Catalizador B2S7 a $c."
        }
    }
