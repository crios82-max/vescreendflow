...
/** Catalyst temp bank 1 sensor 13 (OBD PID 0187), °C. */
object CatalystB1S13 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB1S13 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador B1S13 crítico. $c."
            "warn" -> "Cuidado. Catalizador B1S13 caliente. $c."
            else -> "Catalizador B1S13 a $c."
        }
    }
