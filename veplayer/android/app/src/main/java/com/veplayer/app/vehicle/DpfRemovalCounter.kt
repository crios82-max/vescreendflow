...
object DpfRemovalCounter {
    ...
    fun evaluate(count: Float?, speedKmh: Float = 0f, warnCount: Float = 100f, alertCount: Float = 200f, speedMinKmh: Float = 20f): State {
        if (count == null) return State(band = "idle", label = "")
        val n = count.coerceIn(0f, 65535f)
        val speed = speedKmh.coerceAtLeast(0f)
        val warn = warnCount.coerceIn(10f, 10000f)
        val alert = alertCount.coerceAtLeast(warn + 20f).coerceAtMost(65535f)
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(count = n, speedKmh = speed, band = "ok", label = if (n >= warn - 20f) "DpfRem · ${n.toInt()}" else "")
        }
        val band = when { n >= alert -> "alert"; n >= warn -> "warn"; else -> "ok" }
        return State(count = n, speedKmh = speed, band = band, showWarn = band != "ok", label = "DpfRem · ${n.toInt()}")
    }

    fun voicePhrase(st: State): String {
        val c = st.count?.toInt()?.toString() ?: "alto"
        return when (st.band) {
            "alert" -> "Atención. Contador remoción DPF crítico. $c."
            "warn" -> "Cuidado. Contador remoción DPF alto. $c."
            else -> "Contador remoción DPF a $c."
        }
    }
