...
/** Fuel system 3 use % (OBD PID 019F byte D). */
object FuelSysUsePct3 {
    ...
    fun evaluate(usePct: Float?, speedKmh: Float = 0f, warnPct: Float = 70f, alertPct: Float = 85f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(usePct = p, speedKmh = speed, band = "ok", label = if (p >= warn - 5f) "FSu3 · ${p.toInt()}%" else "")
        }
        val band = when { p >= alert -> "alert"; p >= warn -> "warn"; else -> "ok" }
        return State(usePct = p, speedKmh = speed, band = band, showWarn = band != "ok", label = "FSu3 · ${p.toInt()}%")
    }

    fun voicePhrase(st: State): String {
        val p = st.usePct?.toInt()?.toString() ?: "alto"
        return when (st.band) {
            "alert" -> "Atención. Uso sistema combustible tres crítico. $p por ciento."
            "warn" -> "Cuidado. Uso sistema combustible tres alto. $p por ciento."
            else -> "Uso sistema combustible tres a $p por ciento."
        }
    }
