...
/** Catalyst temp bank 1 sensor 4 (OBD PID 0175), °C. */
object CatalystB1S4 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB1S4 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador B1S4 crítico. $c."
            "warn" -> "Cuidado. Catalizador B1S4 caliente. $c."
            else -> "Catalizador B1S4 a $c."
        }
    }
