...
/** Absolute throttle position G (OBD PID 018D), %. */
object ThrottleG {
    ...
    fun evaluate(throttlePct: Float?, speedKmh: Float = 0f, warnPct: Float = 75f, alertPct: Float = 90f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(throttlePct = thr, speedKmh = speed, band = "ok", label = if (thr >= 45f) "ThrG · ${thr.toInt()}%" else "")
        }
        val band = when { thr >= alert -> "alert"; thr >= warn -> "warn"; else -> "ok" }
        return State(throttlePct = thr, speedKmh = speed, band = band, showWarn = band != "ok", label = "ThrG · ${thr.toInt()}%")
    }

    fun voicePhrase(st: State): String {
        val p = st.throttlePct?.toInt()?.let { "$it por ciento" } ?: "alta"
        return when (st.band) {
            "alert" -> "Atención. Mariposa G crítica. $p."
            "warn" -> "Cuidado. Mariposa G alta. $p."
            else -> "Mariposa G a $p."
        }
    }
