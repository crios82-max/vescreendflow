...
object O2ConcB1S3 {
    ...
    fun evaluate(concPct: Float?, speedKmh: Float = 0f, warnPct: Float = 12f, alertPct: Float = 18f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(concPct = p, speedKmh = speed, band = "ok", label = if (p >= warn - 2f) "O2C3 · ${fmt(p)}%" else "")
        }
        val band = when { p >= alert -> "alert"; p >= warn -> "warn"; else -> "ok" }
        return State(concPct = p, speedKmh = speed, band = band, showWarn = band != "ok", label = "O2C3 · ${fmt(p)}%")
    }
    ...
    fun voicePhrase(st: State): String {
        ...
        return when (st.band) {
            "alert" -> "Atención. O2 concentración B1S3 crítica. $v por ciento."
            "warn" -> "Cuidado. O2 concentración B1S3 alta. $v por ciento."
            else -> "O2 concentración B1S3 a $v por ciento."
        }
    }
