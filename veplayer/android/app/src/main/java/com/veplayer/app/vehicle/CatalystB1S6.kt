...
/** Catalyst temp bank 1 sensor 6 (OBD PID 0179), °C. */
object CatalystB1S6 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB1S6 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador B1S6 crítico. $c."
            "warn" -> "Cuidado. Catalizador B1S6 caliente. $c."
            else -> "Catalizador B1S6 a $c."
        }
    }
