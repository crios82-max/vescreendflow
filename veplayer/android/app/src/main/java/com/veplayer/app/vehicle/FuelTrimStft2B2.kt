...
/** STFT secondary O2 bank 2 (OBD PID 0157), signed %. */
object FuelTrimStft2B2 {
    ...
    fun evaluate(trimPct: Float?, speedKmh: Float = 0f, warnPct: Float = 12f, alertPct: Float = 20f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) return State(trimPct = trim, speedKmh = speed, band = "ok", label = if (abs >= 8f) "ST2B2 · ${formatPct(trim)}" else "")
        val band = when { abs >= alert -> "alert"; abs >= warn -> "warn"; else -> "ok" }
        return State(trimPct = trim, speedKmh = speed, band = band, showWarn = band != "ok", label = "ST2B2 · ${formatPct(trim)}")
    }

    fun voicePhrase(st: State): String {
        val p = st.trimPct?.let { formatPct(it) } ?: "alto"
        return when (st.band) {
            "alert" -> "Atención. STFT O2 secundario B2 crítico. $p."
            "warn" -> "Cuidado. STFT O2 secundario B2 fuera de rango. $p."
            else -> "STFT O2 secundario B2 a $p."
        }
    }
