...
object EssCap {
    ...
    fun evaluate(kwh: Float?, warnKwh: Float = 40f, alertKwh: Float = 25f): State {
        ...
        return State(kwh = k, band = band, showWarn = band != "ok", label = "EssCap · ${"%.1f".format(k)}kWh")
    }

    fun voicePhrase(st: State): String {
        val k = st.kwh?.let { "${"%.1f".format(it)} kilovatios hora" } ?: "baja"
        return when (st.band) {
            "alert" -> "Atención. Capacidad ESS calculada crítica. $k."
            "warn" -> "Cuidado. Capacidad ESS calculada baja. $k."
            else -> "Capacidad ESS calculada a $k."
        }
    }
