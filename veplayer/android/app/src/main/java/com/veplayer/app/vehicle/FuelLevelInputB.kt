...
object FuelLevelInputB {
    ...
    fun evaluate(levelPct: Float?, speedKmh: Float = 0f, warnPct: Float = 15f, alertPct: Float = 8f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(levelPct = pct, speedKmh = speed, band = "ok", label = if (pct <= warn + 5f) "FuelB · ${pct.toInt()}%" else "")
        }
        val band = when { pct <= alert -> "alert"; pct <= warn -> "warn"; else -> "ok" }
        return State(levelPct = pct, speedKmh = speed, band = band, showWarn = band != "ok", label = "FuelB · ${pct.toInt()}%")
    }

    fun voicePhrase(st: State): String {
        val p = st.levelPct?.toInt()?.let { "$it por ciento" } ?: "bajo"
        return when (st.band) {
            "alert" -> "Atención. Nivel combustible B crítico. $p."
            "warn" -> "Cuidado. Nivel combustible B bajo. $p."
            else -> "Nivel combustible B a $p."
        }
    }
