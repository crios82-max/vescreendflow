...
object DriverTorque {
    ...
    fun evaluate(torquePct: Float?, speedKmh: Float = 0f, warnPct: Float = 40f, alertPct: Float = 55f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) return State(torquePct = t, speedKmh = speed, band = "ok", label = if (abs >= 20f) "DrvT · ${t.toInt()}%" else "")
        val band = when { abs >= alert -> "alert"; abs >= warn -> "warn"; else -> "ok" }
        return State(torquePct = t, speedKmh = speed, band = band, showWarn = band != "ok", label = "DrvT · ${t.toInt()}%")
    }

    fun voicePhrase(st: State): String {
        val p = st.torquePct?.toInt()?.let { "$it por ciento" } ?: "alta"
        return when (st.band) {
            "alert" -> "Atención. Demanda de torque crítica. $p."; "warn" -> "Cuidado. Demanda de torque alta. $p."; else -> "Demanda torque a $p."
        }
    }
