...
/** HVESS State of Health % (OBD PID 01BE byte A). Distinct from traction SOH 01B2. */
object HvessSoh {
    ...
    fun evaluate(sohPct: Float?, warnPct: Float = 70f, alertPct: Float = 50f): State {
        ...
        return State(sohPct = p, band = band, showWarn = band != "ok", label = "HvSOH · ${p.toInt()}%")
    }

    fun voicePhrase(st: State): String {
        val p = st.sohPct?.toInt()?.let { "$it por ciento" } ?: "bajo"
        return when (st.band) {
            "alert" -> "Atención. SOH HVESS crítico. $p."
            "warn" -> "Cuidado. SOH HVESS bajo. $p."
            else -> "SOH HVESS a $p."
        }
    }
