...
/** O2 wide-range lambda bank 1 sensor 3 (OBD PID 019C bytes J/K). */
object O2LambdaB1S3 {
    ...
    fun evaluate(lambda: Float?, speedKmh: Float = 0f, warnLambda: Float = 1.10f, alertLambda: Float = 1.15f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(lambda = l, speedKmh = speed, band = "ok", label = if (l >= 1.02f) "O2λ3 · ${fmt(l)}" else "")
        }
        val band = when { l >= alert -> "alert"; l >= warn -> "warn"; else -> "ok" }
        return State(lambda = l, speedKmh = speed, band = band, showWarn = band != "ok", label = "O2λ3 · ${fmt(l)}")
    }
    ...
    fun voicePhrase(st: State): String {
        ...
        return when (st.band) {
            "alert" -> "Atención. Lambda O2 B1S3 crítica. $v."
            "warn" -> "Cuidado. Lambda O2 B1S3 alta. $v."
            else -> "Lambda O2 B1S3 a $v."
        }
    }
