...
/** Catalyst temp bank 1 sensor 5 (OBD PID 0177), °C. */
object CatalystB1S5 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB1S5 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador B1S5 crítico. $c."
            "warn" -> "Cuidado. Catalizador B1S5 caliente. $c."
            else -> "Catalizador B1S5 a $c."
        }
    }
