...
/** Catalyst temp bank 2 sensor 3 (OBD PID 0174), °C. */
object CatalystB2S3 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB2S3 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador B2S3 crítico. $c."
            "warn" -> "Cuidado. Catalizador B2S3 caliente. $c."
            else -> "Catalizador B2S3 a $c."
        }
    }
