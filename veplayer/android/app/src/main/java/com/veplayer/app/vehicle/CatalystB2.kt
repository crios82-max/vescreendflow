...
/** Catalyst temp bank 2 (OBD PID 0170), °C. */
object CatalystB2 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB2 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador banco 2 crítico. $c."; "warn" -> "Cuidado. Catalizador B2 caliente. $c."; else -> "Catalizador B2 a $c."
        }
    }
