...
/** EGT bank 2 sensor 6 (OBD PID 0199 bytes D/E), °C. */
object EgtB2S6 {
    ...
    fun evaluate(egtTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(egtTempC = c, band = band, showWarn = band != "ok", label = "EGTB2S6 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.egtTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. EGT B2S6 crítico. $c."
            "warn" -> "Cuidado. EGT B2S6 caliente. $c."
            else -> "EGT B2S6 a $c."
        }
    }
