...
| Campo | Valor |
|-------|-------|
| URL | `https://ride-api.vescreenflow.com/webhooks/stripe` |
...
```bash
curl -sf https://ride-api.vescreenflow.com/health
...
- **Settings → Payment method domains** → añade `ride.vescreenflow.com`
...
- [ ] Webhook live apuntando a `ride-api.vescreenflow.com`
- [ ] `npm run build` después de cambiar `VITE_STRIPE_PUBLISHABLE_KEY`
- [ ] Pago de prueba real (monto bajo) desde `https://ride.vescreenflow.com`
