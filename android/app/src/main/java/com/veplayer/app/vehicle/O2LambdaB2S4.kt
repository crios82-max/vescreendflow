...
/** O2 wide-range lambda bank 2 sensor 4 (OBD PID 019C bytes P/Q). */
object O2LambdaB2S4 {
    ...
    fun evaluate(lambda: Float?, speedKmh: Float = 0f, warnLambda: Float = 1.10f, alertLambda: Float = 1.15f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(lambda = l, speedKmh = speed, band = "ok", label = if (l >= 1.02f) "O2λ24 · ${fmt(l)}" else "")
        }
        val band = when { l >= alert -> "alert"; l >= warn -> "warn"; else -> "ok" }
        return State(lambda = l, speedKmh = speed, band = band, showWarn = band != "ok", label = "O2λ24 · ${fmt(l)}")
    }
    ...
    fun voicePhrase(st: State): String {
        ...
        return when (st.band) {
            "alert" -> "Atención. Lambda O2 B2S4 crítica. $v."
            "warn" -> "Cuidado. Lambda O2 B2S4 alta. $v."
            else -> "Lambda O2 B2S4 a $v."
        }
    }
