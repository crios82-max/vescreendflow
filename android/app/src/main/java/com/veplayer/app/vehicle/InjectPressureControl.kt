package com.veplayer.app.vehicle

/** Injection pressure control kPa (OBD PID 016E). */
object InjectPressureControl {
    data class State(val pressureKpa: Float? = null, val speedKmh: Float = 0f, val band: String = "idle", val showWarn: Boolean = false, val label: String = "")

    fun evaluate(pressureKpa: Float?, speedKmh: Float = 0f, warnKpa: Float = 8000f, alertKpa: Float = 12000f, speedMinKmh: Float = 10f): State {
        if (pressureKpa == null) return State(band = "idle", label = "")
        val p = pressureKpa.coerceAtLeast(0f)
        val speed = speedKmh.coerceAtLeast(0f)
        val warn = warnKpa.coerceIn(2000f, 20000f)
        val alert = alertKpa.coerceAtLeast(warn + 500f)
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(pressureKpa = p, speedKmh = speed, band = "ok", label = if (p >= 4000f) "Inject · ${p.toInt()}kPa" else "")
        }
        val band = when { p >= alert -> "alert"; p >= warn -> "warn"; else -> "ok" }
        return State(pressureKpa = p, speedKmh = speed, band = band, showWarn = band != "ok", label = "Inject · ${p.toInt()}kPa")
    }

    fun voicePhrase(st: State): String {
        val k = st.pressureKpa?.toInt()?.let { "$it kilopascales" } ?: "alta"
        return when (st.band) {
            "alert" -> "Atención. Presión inyección crítica. $k."
            "warn" -> "Cuidado. Presión inyección alta. $k."
            else -> "Inyección a $k."
        }
    }

    fun accentArgb(band: String): Long = when (band) { "alert" -> 0xFFE11D48; "warn" -> 0xFFF97316; "ok" -> 0xFF14B8A6; else -> 0xFF94A3B8 }

    fun toJsonMap(st: State): Map<String, Any?> = mapOf("pressure_kpa" to st.pressureKpa?.toDouble(), "speed_kmh" to st.speedKmh.toDouble(), "band" to st.band, "show_warn" to st.showWarn, "label" to st.label)
}
