...
/** Catalyst temp bank 1 sensor 12 (OBD PID 0185), °C. */
object CatalystB1S12 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB1S12 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador B1S12 crítico. $c."
            "warn" -> "Cuidado. Catalizador B1S12 caliente. $c."
            else -> "Catalizador B1S12 a $c."
        }
    }
