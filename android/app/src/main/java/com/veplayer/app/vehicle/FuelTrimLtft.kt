...
/**
 * Long-term fuel trim bands (OBD PID 0107), signed %.
 */
object FuelTrimLtft {
    data class State(
    ...
    fun evaluate(
        ...
        if (speed < minSpd) {
            return State(
                ...
                speedKmh = speed,
                band = "ok",
                label = if (absTrim >= 8f) "LTFT · ${formatTrimPct(trim)}" else "",
            )
        }
    ...
    fun voicePhrase(st: State): String {
        ...
        return when (st.band) {
            "alert" -> "Atención. Corrección de combustible LTFT crítica. $p. Revisa motor."
            "warn" -> "Cuidado. Corrección LTFT fuera de rango. $p."
            else -> "Corrección LTFT a $p."
        }
    }
