...
/** Catalyst temp bank 2 sensor 14 (OBD PID 018A), °C. */
object CatalystB2S14 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB2S14 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador B2S14 crítico. $c."
            "warn" -> "Cuidado. Catalizador B2S14 caliente. $c."
            else -> "Catalizador B2S14 a $c."
        }
    }
