...
/** ESS actual charging power kW (OBD PID 01D1 bytes C/D signed /10). High |kW| = stress. */
object EssChgAct {
    ...
    fun evaluate(kw: Float?, warnKw: Float = 80f, alertKw: Float = 120f): State {
        ...
        return State(kw = kw, band = band, showWarn = band != "ok", label = "EssAct · ${"%.1f".format(kw)}kW")
    }

    fun voicePhrase(st: State): String {
        val k = st.kw?.let { "${"%.1f".format(it)} kilovatios" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Potencia de carga ESS crítica. $k."
            "warn" -> "Cuidado. Potencia de carga ESS alta. $k."
            else -> "Potencia de carga ESS a $k."
        }
    }
