...
/** Catalyst temp bank 1 sensor 9 (OBD PID 017F), °C. */
object CatalystB1S9 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB1S9 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador B1S9 crítico. $c."
            "warn" -> "Cuidado. Catalizador B1S9 caliente. $c."
            else -> "Catalizador B1S9 a $c."
        }
    }
