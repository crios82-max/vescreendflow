...
/** PM sensor normalized output B1S1 % (OBD PID 018F bytes C/D). */
object PmSensorB1 {
    ...
    fun evaluate(pmPct: Float?, speedKmh: Float = 0f, warnPct: Float = 70f, alertPct: Float = 85f, speedMinKmh: Float = 15f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(pmPct = p, speedKmh = speed, band = "ok", label = if (p >= 45f) "PMB1 · ${p.toInt()}%" else "")
        }
        val band = when { p >= alert -> "alert"; p >= warn -> "warn"; else -> "ok" }
        return State(pmPct = p, speedKmh = speed, band = band, showWarn = band != "ok", label = "PMB1 · ${p.toInt()}%")
    }

    fun voicePhrase(st: State): String {
        val p = st.pmPct?.toInt()?.let { "$it por ciento" } ?: "alto"
        return when (st.band) {
            "alert" -> "Atención. PM B1 crítico. $p."
            "warn" -> "Cuidado. PM B1 alto. $p."
            else -> "PM B1 a $p."
        }
    }
