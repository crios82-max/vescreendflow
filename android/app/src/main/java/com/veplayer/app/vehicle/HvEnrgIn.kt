...
object HvEnrgIn {
    ...
    fun evaluate(kwh: Float?, warnKwh: Float = 15000f, alertKwh: Float = 25000f): State {
        ...
        return State(kwh = k, band = band, showWarn = band != "ok", label = "HvIn · ${k.toInt()}kWh")
    }

    fun voicePhrase(st: State): String {
        val k = st.kwh?.toInt()?.let { "$it kWh" } ?: "elevado"
        return when (st.band) {
            "alert" -> "Atención. Energía acumulada HV crítica. $k."
            "warn" -> "Cuidado. Energía acumulada HV alta. $k."
            else -> "Energía acumulada HV a $k."
        }
    }
