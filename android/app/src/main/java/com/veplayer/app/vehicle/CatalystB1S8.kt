...
/** Catalyst temp bank 1 sensor 8 (OBD PID 017D), °C. */
object CatalystB1S8 {
    ...
    fun evaluate(catalystTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(catalystTempC = c, band = band, showWarn = band != "ok", label = "CatB1S8 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.catalystTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. Catalizador B1S8 crítico. $c."
            "warn" -> "Cuidado. Catalizador B1S8 caliente. $c."
            else -> "Catalizador B1S8 a $c."
        }
    }
