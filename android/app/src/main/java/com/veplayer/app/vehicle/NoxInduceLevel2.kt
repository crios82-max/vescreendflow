...
object NoxInduceLevel2 {
    ...
    fun evaluate(status: Int?, speedKmh: Float = 0f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(status = s, speedKmh = speed, band = "ok", label = if (s >= 1) "IndL2 · $s" else "")
        }
        val band = when (s) { 2 -> "alert"; 1 -> "warn"; else -> "ok" }
        return State(status = s, speedKmh = speed, band = band, showWarn = band != "ok", label = if (band != "ok") "IndL2 · $s" else "")
    }

    fun voicePhrase(st: State): String = when (st.band) {
        "alert" -> "Atención. Inducement NOx nivel dos activo."
        "warn" -> "Cuidado. Inducement NOx nivel dos habilitado."
        else -> "Inducement NOx nivel dos estado ${st.status ?: 0}."
    }
