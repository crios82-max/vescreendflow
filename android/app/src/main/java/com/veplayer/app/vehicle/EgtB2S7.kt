...
/** EGT bank 2 sensor 7 (OBD PID 0199 bytes F/G), °C. */
object EgtB2S7 {
    ...
    fun evaluate(egtTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(egtTempC = c, band = band, showWarn = band != "ok", label = "EGTB2S7 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.egtTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. EGT B2S7 crítico. $c."
            "warn" -> "Cuidado. EGT B2S7 caliente. $c."
            else -> "EGT B2S7 a $c."
        }
    }
