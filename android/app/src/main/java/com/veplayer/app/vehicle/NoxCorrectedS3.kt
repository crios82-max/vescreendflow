...
object NoxCorrectedS3 {
    ...
    fun evaluate(noxPpm: Float?, speedKmh: Float = 0f, warnPpm: Float = 600f, alertPpm: Float = 800f, speedMinKmh: Float = 20f): State {
        ...
        if (speed < speedMinKmh.coerceIn(0f, 60f)) {
            return State(noxPpm = ppm, speedKmh = speed, band = "ok", label = if (ppm >= warn - 50f) "NOxC3 · ${ppm.toInt()}" else "")
        }
        val band = when { ppm >= alert -> "alert"; ppm >= warn -> "warn"; else -> "ok" }
        return State(noxPpm = ppm, speedKmh = speed, band = band, showWarn = band != "ok", label = "NOxC3 · ${ppm.toInt()}")
    }

    fun voicePhrase(st: State): String {
        val p = st.noxPpm?.toInt()?.let { "$it ppm" } ?: "elevado"
        return when (st.band) {
            "alert" -> "Atención. NOx corregido S3 crítico. $p."
            "warn" -> "Cuidado. NOx corregido S3 alto. $p."
            else -> "NOx corregido S3 a $p."
        }
    }
