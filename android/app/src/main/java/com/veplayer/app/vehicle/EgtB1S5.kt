...
/** EGT bank 1 sensor 5 (OBD PID 0198 bytes B/C), °C. */
object EgtB1S5 {
    ...
    fun evaluate(egtTempC: Float?, warnC: Float = 750f, alertC: Float = 850f): State {
        ...
        return State(egtTempC = c, band = band, showWarn = band != "ok", label = "EGTB1S5 · ${c.toInt()}°C")
    }

    fun voicePhrase(st: State): String {
        val c = st.egtTempC?.toInt()?.let { "$it grados" } ?: "elevada"
        return when (st.band) {
            "alert" -> "Atención. EGT B1S5 crítico. $c."
            "warn" -> "Cuidado. EGT B1S5 caliente. $c."
            else -> "EGT B1S5 a $c."
        }
    }
