...
for (const hex of pollKotlin) {
  ...
  if (!Object.keys(sample).length && byte !== 0x0d) {
    ...
    const twoByte = [0x0c, 0x10, 0x34, 0x43, 0x44, 0x53, 0x59, 0x70, 0x71, 0x72, 0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x5d, 0x63, 0x1f, 0x21, 0x31, 0x42, 0x5e, 0x7f, 0x80, 0x81, 0x82, 0x83, 0x84, 0x85, 0x86, 0x6d, 0x6e]
    const threeByte = [0x67, 0x68]
    const twoByteMin2 = [0x6b]
    if (threeByte.includes(byte)) {
      const got = parseMode01(`41 ${byte.toString(16).padStart(2, '0')} 00 50 8C`)
